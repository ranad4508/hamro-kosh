          ...
          return ListView(
            ...
            children: [
              if (pending.isNotEmpty) ...[
                Text('Pending approval', style: Theme.of(context).textTheme.titleSmall),
...
class _MemberTile extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    Future<void> run(Future<void> Function() action, {required String successMessage}) async {
      try {
        await action();
        if (context.mounted) {
          AppSnackbar.showSuccess(context, title: 'Updated', message: successMessage);
        }
      } catch (_) {
        if (context.mounted) {
        ...
        trailing: !member.isApproved
            ...
            : PopupMenuButton<bool>(
                onSelected: (active) => run(
                  ...
                  successMessage: '${member.fullName} ${active ? 'activated' : 'deactivated'}.',
                ),
                itemBuilder: (context) => [
